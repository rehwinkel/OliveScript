...
fn write_to_file(pathname: &String, codes: &Vec<u8>) -> IoResult<()> {
...
fn main() -> Result<(), String> {
    ...
    if args.len() == 2 {
        ...
        let contents: String =
            fs::read_to_string(path.file_name().unwrap()).map_err(|err| format!("{}", err))?;
        ...
        write_to_file(&outpath, &codes).map_err(|err| format!("{}", err))?;
        Ok(())
    } else {
